...
class PluginManager {
  constructor(davis) {
    ...
    this.hooks = {};
    this.options = {};
    this.ymonitor = new Ymonitor(davis);
  ...
  loadEntities() {
    return BbPromise.all([
      ...
      this.ymonitor.getKetens(),
    ])
      .spread((applications, services, ketens) => {
        this.logger.info(`Found ${ketens.length} applications running ${ketens.length} services.`);
        return BbPromise.all([
          this._saveEntities(applications, 'applications'),
          this._saveEntities(services, 'services'),
          this._saveYmonitorKetens(ketens, 'applications'),
        ]);
      })
      .then(() => this.logger.info('Finished loading entities.'))
      .catch((e) => this.logger.warn('Unable to load entities.' +   e));
  }
  ...
  _saveEntities(entities, category) {
    ...
    .then((entities) => {
      entities.filter(e => e.category == 'applications').forEach(e => console.log(e));
  ...
  _saveYmonitorKetens(ketens, category) {
    this.logger.debug('saveYmonitorKetens ' + ketens.size);
    return BbPromise.map(ketens, entity =>
      AliasesModel.findOne({ entityId: entity.id })
        .exec()
        .then((alias) => {
          if (!_.isNil(alias)) {
            alias.aliases = alias.aliases.filter(a => a.toLowerCase() !== alias.name.toLowerCase());
