  ...
  constructor(davis, options) {
    ...
    this.hooks = {
    		...
    		'ymonitor:authenticate': (exchange, context) => {
    			 ...
    			 const username = this.config.getYmonitorUsername();
    			 const password = this.config.getYmonitorPassword();
    			 
    			const bodyText = new String ("\"username\" : " +"\"" + username + "\"," + "\"password\" : " + "\"" + password + "\"" );
    			davis.logger.debug(bodyText); 
    			// davis.logger.debug("username": this.username + "," + "password" ":" + "${password}"); 
  			  ...
  			  const opts = {
  			    ...
  			    body: {
  			    	bodyText
  			    	// insert password here "username": "${username}", "password": "${password}"
  			    },
  			    json: true,
  			  }
  			davis.logger.debug(opts); 
  			  return rp(opts)
  			    .then(resp => {
