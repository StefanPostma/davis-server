  		...
  		'Ymonitor:gatherData': (exchange, context) => {
			  ...
			  return rp(opts)
			    .then(resp => {
			      ...
			      var myJson = resp['keten'];
			      davis.logger.debug(JSON.stringify(myJson));
			     
			      for(var myKey in myJson) {
			    	  davis.logger.debug("key:"+myKey+", value:"+JSON.stringify(myJson[myKey]));
			    	//  console.log("key:"+myKey+", value:"+myJson[myKey]);
			    	  var data = JSON.parse(JSON.stringify(myJson[myKey])); // you missed that...
			    	  davis.logger.debug(data.length);
			    	  for(var i = 0; i < data.length; i++) {
				    	  davis.logger.debug("In the Array! hurrahh");
				    	  exchange.addContext({
						        id: data[i].id,
						        displayName: data[i].displayName,
						      })
						      davis.logger.debug(context.displayName);
				      }
			    	}
