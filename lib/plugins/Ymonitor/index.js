  ...
  constructor(davis, options) {
    ...
    this.hooks = {
			  ...
			  return rp(opts)
			    .then(resp => {
			      ...
			      for(var myKey in myJson) {
			    	  davis.logger.debug("key:"+myKey+", value:"+ JSON.stringify(myJson[myKey]));
			    	  var myNewJson = myJson[myKey];
			    	  for(var myKey in myNewJson) {
			    		  exchange.response(JSON.stringify(myNewJson[myKey]));
			    		  exchange.followUp('Would you like to hear the next chain?');
			    		  davis.logger.debug("key:"+myKey+", value:"+ JSON.stringify(myNewJson[myKey]));
      ...
      'Ymonitor:respond': (exchange, context) => {
        let out = 'The following chains have been found: ';
		out += context.keten.displayName;
		out += ' and ';
		out += context.keten.displayName;
		out += '.';
