  		...
  		'Ymonitor:gatherData': (exchange, context) => {
			  ...
			  return rp(opts)
			    .then(resp => {
			      ...
			      var a = JSON.parse(JSON.stringify(context.keten));
			      davis.logger.debug(JSON.stringify(context.keten));
			      davis.logger.debug(JSON.stringify(a));
			      davis.logger.debug(JSON.stringify(a[displayName]));
			    })
		},
