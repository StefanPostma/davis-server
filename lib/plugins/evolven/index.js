  ...
  constructor(davis, options) {
    ...
    this.hooks = {
    		...
    		'evolven:authenticate': (exchange, context) => {
    			...
    			//davis.logger.debug("Evolven User: " + this.config.getEvolvenUsername() +  "\nEvolven Password: " +  this.config.getEvolvenPassword() + "\n Evolven url: " +  this.config.getEvolvenUrl()); 
    			var url = this.config.getEvolvenUrl() +"?action=login&user=" + this.config.getEvolvenUsername() +"&pass=" + this.config.getEvolvenPassword();
    			//davis.logger.debug("the request url:" + url);
  			...
  			davis.logger.debug(JSON.stringify(opts)); 
  			  return rp(opts)
  			    .then(resp => {
  			    var xmlData = et.parse(resp);
  			  davis.logger.debug("the XML parsed : " + JSON.stringify(xmlData));
  		        var ID = xmlData.findtext('./Next/ID');
  		        davis.logger.debug("the ID Found: " + ID);
