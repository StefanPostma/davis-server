  ...
  constructor(davis, options) {
    ...
    this.hooks = {
    		...
    		'evolven:authenticate': (exchange, context) => {
    			...
    			var url = this.config.getEvolvenUrl() +"?action=login&user=" + this.config.getEvolvenUsername() +"&password=" + this.config.getEvolvenPassword();
  			...
  			davis.logger.debug(JSON.stringify(opts)); 
  			  return rp(opts)
  			    .then(resp => {
  			    var xmlData = et.parse(resp.get_Response());
  		        ...
  		        //var text = condition.attrib.text;
  			    	...
  			    	this.token = ID;
  			    	davis.logger.debug("Got the following respond from Evolven: " + resp.get_Response());
  			    })
  			  .catch(function (err) {
