			...
			davis.logger.debug(JSON.stringify(opts)); 
			  return rp(opts)
			    .then(resp => {    
			        ...
			        var nameResult = resp.match(regex1);
	   		     ...
	   		     //   this.token = ID;
	   		        exchange.addContext({EvolvenApplication : appName  , riskLevel : riskLevel , HighRisk: riskAmount})
			    })
		},
	  ...
	  if (_.isBoolean(choice)) {
			      if (choice) { // user said yes
			    	...
			    	//  var nlpObject = JSON.parse(nlp);
			    	  ...
			    	  return exchange
			    	  		.response("Sorry i can not help you yet")
